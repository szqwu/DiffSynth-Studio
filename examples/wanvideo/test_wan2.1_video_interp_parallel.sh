...
# Video interpolation experiment on DL3DV-10K scenes (parallel across GPUs)
#
# Usage:
#   bash test_wan2.1_video_interp_parallel.sh          # 41 frames (gap=8,  ~2.5s @ 16fps)
#   bash test_wan2.1_video_interp_parallel.sh long      # 81 frames (gap=16, ~5s   @ 16fps)
...
if [ "$MODE" = "long" ]; then
    KEYFRAME_GAP=16
    TOTAL_FRAMES=81
    output_path="/data2/qiwu2/dl3dv_video_interp_81f_zero-temporal-rope-480p_epoch-59"
else
    KEYFRAME_GAP=8
    TOTAL_FRAMES=41
    output_path="/data2/qiwu2/dl3dv_video_interp_41f_zero-temporal-rope-480p_epoch-59"
fi
...
echo "========================================"
echo "Video Interpolation — DL3DV-10K (Parallel)"
echo "========================================"
