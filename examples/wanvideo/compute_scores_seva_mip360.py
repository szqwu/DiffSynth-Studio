...
# coding=utf-8
"""
Compute PSNR, SSIM, LPIPS, and DreamSim scores for SEVA 6-to-N NVS results
on the mip-NeRF 360 dataset.

Two evaluation modes (matching eval_6to1_nvs_mip360.py):

  precrop=True  (default, matches the eval script):
    GT:   images_2 → resize+center-crop to eval_h x eval_w
    Pred: SEVA 576x576 output → resize to eval_h x eval_w

  precrop=False:
    GT:   images_2 → resize+center-crop to SEVA_SIZE x SEVA_SIZE → resize to eval_h x eval_w
    Pred: SEVA 576x576 output → resize to eval_h x eval_w

Usage:
    python compute_scores_seva_mip360.py \
        --results_dir /data2/qiwu2/mip360_test_results_SEVA

    python compute_scores_seva_mip360.py \
        --results_dir /data2/qiwu2/mip360_test_results_SEVA \
        --precrop --eval_h 480 --eval_w 480
"""
...
# ──────────────────────────────────────────────────────────────────────────────
# COLMAP reader (only images.bin for sorted name list)
# ──────────────────────────────────────────────────────────────────────────────
...
def resize_and_center_crop(img, target_h, target_w=None):
    """Resize so the image covers target_h x target_w, then center crop."""
    if target_w is None:
        target_w = target_h
    ...
    return center_crop(img_resized, target_h, target_w)


def load_gt_frame(scene_data_path, image_name, target_h, target_w):
    """Load GT from images_2/ and resize+center-crop to target_h x target_w."""
    ...
    return resize_and_center_crop(img, target_h, target_w)


def load_pred_frame(scene_dir, target_pos, eval_h, eval_w):
    """Load SEVA output and resize to eval size."""
    ...
    img = np.array(Image.open(pred_path).convert("RGB"))
    if img.shape[0] != eval_h or img.shape[1] != eval_w:
        img = cv2.resize(img, (eval_w, eval_h), interpolation=cv2.INTER_AREA)
    return img
...
def save_comparison_image(gt_img, pred_img, frame_idx, image_name,
                          psnr, ssim_val, lpips_val, ds_val, save_path, eval_h, eval_w):
    bar_h = 40
    gap = 10
    canvas_w = eval_w * 2 + gap
    canvas_h = eval_h + bar_h
    comp = np.full((canvas_h, canvas_w, 3), 255, dtype=np.uint8)

    comp[:eval_h, :eval_w] = gt_img
    comp[:eval_h, eval_w + gap:] = pred_img

    font = cv2.FONT_HERSHEY_SIMPLEX
    cv2.putText(comp, "GT", (10, 30), font, 0.7, (0, 200, 0), 2, cv2.LINE_AA)
    cv2.putText(comp, "Pred", (eval_w + gap + 10, 30), font, 0.7, (0, 200, 200), 2, cv2.LINE_AA)

    metrics_txt = (f"{image_name} (idx {frame_idx}):  PSNR={psnr:.2f}  SSIM={ssim_val:.4f}  "
                   f"LPIPS={lpips_val:.4f}  DS={ds_val:.4f}")
    cv2.putText(comp, metrics_txt, (10, eval_h + 28), font, 0.45,
                (0, 0, 0), 1, cv2.LINE_AA)
...
def process_scene(scene_name, results_dir, mip360_data_path, mip360_split_path,
                  ...
                  seva_size, eval_h, eval_w, precrop, save_comparisons=False):
    ...
    comp_dir = None
    if save_comparisons:
        comp_dir = os.path.join(scene_dir, f"comparisons_{eval_h}x{eval_w}")
        os.makedirs(comp_dir, exist_ok=True)

    psnrs, ssims, lpipss, dreamsims = [], [], [], []
    per_frame = []

    for target_pos, frame_idx in enumerate(tqdm(test_ids, desc=f"  {scene_name}", leave=False)):
        ...
        try:
            if precrop:
                gt_img = load_gt_frame(scene_data_path, img_name, eval_h, eval_w)
            else:
                gt_img = load_gt_frame(scene_data_path, img_name, seva_size, seva_size)
                gt_img = cv2.resize(gt_img, (eval_w, eval_h), interpolation=cv2.INTER_AREA)

            pred_img = load_pred_frame(scene_dir, target_pos, eval_h, eval_w)
        except FileNotFoundError as e:
            print(f"    [SKIP] {e}")
            continue

        assert gt_img.shape == (eval_h, eval_w, 3), f"GT shape mismatch: {gt_img.shape}"
        assert pred_img.shape == (eval_h, eval_w, 3), f"Pred shape mismatch: {pred_img.shape}"
        ...
        if comp_dir is not None:
            save_comparison_image(
                gt_img, pred_img, frame_idx, img_name,
...
def main():
    parser = argparse.ArgumentParser(
        description="Compute PSNR, SSIM, LPIPS, DreamSim on SEVA NVS results (mip-NeRF 360)"
    )
    parser.add_argument("--results_dir", type=str,
                        default="/data2/qiwu2/mip360_test_results_SEVA")
    ...
    parser.add_argument("--seva_size", type=int, default=576,
                        help="SEVA model output resolution (crop mode → NxN)")
    parser.add_argument("--eval_h", type=int, default=480)
    parser.add_argument("--eval_w", type=int, default=480)
    parser.add_argument("--precrop", action="store_true", default=True,
                        help="(default) GT: images_2 → resize+crop to eval_h x eval_w directly")
    parser.add_argument("--no_precrop", action="store_true",
                        help="GT: images_2 → resize+crop to seva_size → resize to eval size")
    parser.add_argument("--output_json", type=str, default=None)
    parser.add_argument("--save_comparisons", action="store_true",
                        help="Save side-by-side GT vs Pred comparison images")
    args = parser.parse_args()

    precrop = not args.no_precrop
    seva_size = args.seva_size
    eval_h = args.eval_h
    eval_w = args.eval_w
    eval_tag = f"{eval_h}x{eval_w}"

    if args.output_json is None:
        mode_tag = "precrop" if precrop else "sevacrop"
        args.output_json = os.path.join(args.results_dir, f"scores_{mode_tag}_{eval_tag}.json")
    ...
    if precrop:
        print(f"Evaluation pipeline (precrop mode):")
        print(f"  GT:   images_2 -> resize+center-crop to {eval_h}x{eval_w}")
        print(f"  Pred: {seva_size}x{seva_size} SEVA output -> resize to {eval_h}x{eval_w}")
    else:
        print(f"Evaluation pipeline (seva-crop mode):")
        print(f"  GT:   images_2 -> resize+center-crop to {seva_size}x{seva_size} -> resize to {eval_h}x{eval_w}")
        print(f"  Pred: {seva_size}x{seva_size} SEVA output -> resize to {eval_h}x{eval_w}")
    ...
    for scene_name in scene_names:
        result = process_scene(
            ...
            ssim_fn, lpips_model, dreamsim_model, dreamsim_preprocess,
            seva_size, eval_h, eval_w, precrop,
    ...
    # ── Overall summary ──────────────────────────────────────────────────
    print(f"\n{'=' * 80}")
    mode_str = "precrop" if precrop else "seva-crop"
    print(f"OVERALL RESULTS  ({len(all_results)} scenes, SEVA {seva_size}x{seva_size} "
          f"-> {eval_tag}, {mode_str})")
    print(f"{'=' * 80}")
    ...
    output = {
        "config": {
            ...
            "eval_resolution": eval_tag,
            "precrop": precrop,
            "gt_pipeline": (
                f"images_2 -> resize+center-crop to {eval_h}x{eval_w}"
                if precrop else
                f"images_2 -> resize+center-crop to {seva_size}x{seva_size} -> resize to {eval_h}x{eval_w}"
            ),
            "pred_pipeline": f"{seva_size}x{seva_size} -> resize to {eval_h}x{eval_w}",
    ...
    # ── Summary text ─────────────────────────────────────────────────────
    summary_path = os.path.join(args.results_dir, f"scores_{mode_str}_{eval_tag}_summary.txt")
    with open(summary_path, "w") as f:
        f.write(f"SEVA mip-NeRF 360 evaluation at {eval_tag} ({mode_str})\n")
        if precrop:
            f.write(f"  GT:   images_2 -> resize+center-crop to {eval_h}x{eval_w}\n")
        else:
            f.write(f"  GT:   images_2 -> resize+center-crop to {seva_size}x{seva_size} "
                    f"-> resize to {eval_h}x{eval_w}\n")
        f.write(f"  Pred: {seva_size}x{seva_size} -> resize to {eval_h}x{eval_w}\n")
