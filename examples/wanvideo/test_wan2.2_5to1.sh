# checkpoint_path="./models/train/Wan2.2-TI2V-5B_lora128_SE_6to1/epoch-25.safetensors"
checkpoint_path="./models/train/Wan2.2-TI2V-5B_lora128_SE_5to1/epoch-25.safetensors"
data_path="/data2/qiwu2/dl3dv_test_SE_train_val"
# data_path="/data2/qiwu2/dl3dv_test_bench_plucker_separate_encoding"
output_dir="wan_se_output_lora128_5to1"

CUDA_VISIBLE_DEVICES=6 python test_wan2.2_5to1.py \
    --checkpoint_path $checkpoint_path \
    --data_path $data_path \
    --output_dir $output_dir \
    --use_dreamsim