...
def main(args):
    ...
    for scene_idx, scene_dir in enumerate(scene_dirs):
        ...
        # ── Evaluate target frame(s) at model resolution ──────────────
        scene_psnrs = []
        scene_dreamsim = []

        for i, target_idx in enumerate(target_indices):
            pred_pil = video[target_idx]  # already at model resolution
            pred_frame = np.array(pred_pil)

            # Resize GT to model resolution for fair comparison
            gt_pil_resized = target_gt_images[i].resize((model_w, model_h), Image.BILINEAR)
            gt_frame = np.array(gt_pil_resized)
            ...
            # Save comparison image at model resolution (GT | Prediction)
