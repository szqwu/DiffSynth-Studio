...
accelerate launch model_training/train_SE.py \
  ...
  --output_path "./models/train/Wan2.1-SE-14B-lora32-zero-temporal-rope_eschernet-combined-8GPU" \
  --lora_base_model "dit" \
  ...
  --wandb_run_name "lora32-zero-temporal-rope-eschernet-combined-8GPU" \
  --num_input_frames 6 \
  --num_output_frames 1 \
