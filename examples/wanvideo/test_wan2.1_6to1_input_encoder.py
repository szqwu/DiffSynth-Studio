"""
Wan2.1-T2V-14B novel view synthesis inference (6-to-1) for the INPUT-ENCODER model
with TRANSFORM-THROUGH-LAYERS + PREFIX ATTENTION.

This matches the training config in
  examples/wanvideo/model_training/lora/Wan2.1-T2V-14B_InputEncoder_6to1.sh
i.e. the 6 conditioning frames are encoded by the FROZEN main VAE (no residual
MLP) and then transform through all DiT layers like normal tokens, but a prefix
attention mask makes context tokens attend only to context while the target
attends to everything. With per-token timestep (context=0, target=t) the context
is a step-invariant clean prefix.

Transform-once payoff: because the context sub-sequence is independent of the
noisy target and of t, its per-layer hidden states are identical across all
denoising steps. Correctness therefore holds even without caching (the context is
simply recomputed but produces identical values every step); a per-layer context
K/V cache is the intended follow-up optimization and does not change results.

Data / scene handling and metrics mirror test_wan2.2_6to1_input_encoder.py so it
can be driven by a parallel launcher (see test_wan2.1_6to1_input_encoder_parallel.sh).
"""
import os
import sys
import time
...
def modify_model_channels(pipe, model_attr, new_in_dim, device):
    ...
    "prefix attention" configuration, matching train_SE.py's modify_model_channels
    with --use_input_encoder --input_prefix_attention --no_input_latent_mlp
    --fuse_vae_embedding_in_latents_multiple --seperated_encoding.
    """
    ...
    new_model = WanModel(
        ...
        # 14B base has seperated_timestep=False; per-token timestep (context=0,
        # target=t) with fused input latents needs it True (training forces this).
        seperated_timestep=True,
        # Input-encoder mode conditions via the fused clean latent slots, so the
        # redundant I2V-style `y` (mask + per-frame VAE latents, +20 ch) is disabled;
        # new_in_dim = latent(16) + raymap(384) = 400.
        ...
        use_input_latent_mlp=False,          # frozen-VAE setup: no residual MLP
        per_layer_input_replacement=False,   # transform through layers
        input_prefix_attention=True,         # context-only attention mask
    )
    ...
    print(f"Model {model_attr} channels modified successfully "
          f"(use_input_encoder=True, input_prefix_attention=True, no MLP)")


def load_pipeline(args):
    """Load the Wan2.1-T2V-14B pipeline, set up the input encoder, and load the checkpoint."""
    # The umt5-xxl text encoder is ~11GB. It is only needed once (empty prompt), so
    # stream it from CPU (offload/onload/preparing on CPU, compute on CUDA) to free
    # memory for the large 14B DiT (~28GB bf16). The DiT and VAE stay resident.
    ...
    pipe = WanVideoPipeline.from_pretrained(
        ...
        device="cuda",
        model_configs=[
            ModelConfig(model_id="Wan-AI/Wan2.1-T2V-14B", origin_file_pattern="models_t5_umt5-xxl-enc-bf16.pth", **text_encoder_offload),
    ...
    # Recreate the DiT with the trained input dim + input-encoder prefix-attention config.
    modify_model_channels(pipe, "dit", args.new_in_dim, "cuda")
    pipe.dit.use_input_encoder = True
    pipe.dit.per_layer_input_replacement = False
    pipe.dit.input_prefix_attention = True

    # Frozen input VAE: reuse the main (frozen) VAE directly — no separate copy.
    pipe.input_vae = pipe.vae
    print("Using the frozen main VAE for input-frame conditioning (input_vae aliased to vae)")
    ...
    # patch_embedding is trained as full weights under pipe.dit.* (saved with the
    # 'pipe.dit.' prefix stripped -> 'patch_embedding.*'). No input_latent_mlp in
    # this setup (frozen VAE, MLP dropped).
...
def prepare_raymap(extrinsics, intrinsics, context_indices, target_indices, height, width,
                   no_pixel_unshuffle=False, downsample_factor=8):
    """
    Normalize camera poses and compute plucker rays at the Wan2.1 VAE /8 resolution.
    NOTE: intrinsics must already be scaled to (height, width) before calling.
    Returns raymap [N, C, H/8, W/8], normalized c2w poses, and intrinsics tensor.
    """
...
def process_scene(pipe, args, scene_hash, scene_idx, total_scenes,
    ...
    for batch_pos, target_batch in enumerate(target_batches):
        ...
        if args.aat_frame_attention:
            # AAT: even blocks = within-frame attention, odd blocks = global. The
            # prefix mask is applied ONLY on the global (odd) blocks (model_fn routes
            # it there and passes None to the within-frame blocks).
            pipe_kwargs["aat_frame_attention"] = True
        if args.transform_once:
            # Transform-once: encode+transform the clean context ONCE (step 0), cache
            # per-layer context K/V, and run only the target token on later steps.
            pipe_kwargs["transform_once"] = True
    ...
    with open(metrics_file, 'w') as f:
        ...
        f.write(f"Method: {num_input}-to-{num_output} generation (input-encoder / prefix attention)\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Wan2.1-T2V-14B input-encoder (prefix attention) 6-to-1 NVS evaluation on DL3DV-10K")

    # Model
    parser.add_argument("--checkpoint_path", type=str, required=True,
                        help="Path to the trained checkpoint (.safetensors)")
    parser.add_argument("--new_in_dim", type=int, default=400,
                        help="Input dim of the modified model (16 latent + 384 raymap = 400 for the 14B).")
    ...
    parser.add_argument("--eval_size", type=int, default=192,
                        help="Evaluation size for center-crop metrics (default: 192)")
    ...
    parser.add_argument("--raymap_downsample_factor", type=int, default=8,
                        help="PixelUnshuffle downscale for raymaps (8 for Wan2.1 VAE -> 384 ch).")
    ...
    parser.add_argument("--aat_frame_attention", action="store_true", default=False,
                        help="Enable AAT alternating attention (even=within-frame, odd=global). "
                             "Must match training --aat_frame_attention. With the prefix model the "
                             "mask is applied on the global blocks only. Compatible with "
                             "--transform_once: context K/V are cached at the global (odd) blocks "
                             "and even blocks run target-only within-frame attention on decode.")
    ...
    parser.add_argument("--no_offload_text_encoder", action="store_true",
                        help="Keep the text encoder resident on GPU (default: stream it from "
                             "CPU to leave room for the large 14B DiT).")
    parser.add_argument("--transform_once", action="store_true",
                        help="Enable the transform-once context K/V cache: the 6 clean context "
                             "frames are transformed through the DiT ONCE (step 0) and their "
                             "per-layer K/V cached; later denoising steps run only the target "
                             "token attending to the cached context. Requires cfg_scale=1.0 and "
                             "the prefix-attention model. Produces identical results to the full "
                             "recompute (verified in _smoke_test_input_encoder_prefix.py).")
